...
void ParticleCollection::computeForcesAndTorques(const PairList pairs_, double side_,size_t max_site_type_
                                                     ) {
  double xab,xa,xb,xij;
  double yab,ya,yb,yij;
  double zab,za,zb,zij;
  double raLsq,rbLsq;
  double raL,raNL,rbL,rbNL;
  double rijL,rijNL,rijLsq,rijLInv1,rijLInv2, rijLInv6, rijLInv12;
  double rijNLsq,rijNLInv1,rijNLInv2, rijNLInv6, rijNLInv12;
  double fabx,fijx,fx;
  double faby,fijy;
  double fabz,fijz;
  ...
  double alpha, P_0, Pi,Pj,epsi,epsiRep,B1,B;
    ...
    for (PairList::const_iterator it = pairs_.begin(); it < pairs_.end(); it++) {
        ...
        Particle& pi = this->at(it->first);
        SiteCollection& si = pi.sites;
//        Particle& pj = this->at(it->first);
//        SiteCollection& sj = pj.sites;

        Particle& pj = this->at(it->second);
        SiteCollection& sj = pj.sites;     
//        Particle& pi = this->at(it->second);
//        SiteCollection& si = pi.sites;     
...
//        Site& sb = sj.at(b);
        ...
        for (size_t a = 0; a < si.size(); a++) {
         ...
         mic(xa, ya, za, side_);
         mic(xij, yij, zij, side_); //added
        
         D3Vector<double> ra_( xa, ya, za ), rij_( xij, yij, zij);
          ra_.normalize();
          rij_.normalize();
  ...
  // March 14 for debugging
        ...
        for (size_t b = 0; b < sj.size(); b++) {
         ...
         D3Vector<double> rb_( xb, yb, zb ), rij_( xij, yij, zij);
          rb_.normalize();
          rij_.normalize();
          Pj = -rb_.dotp(rij_); //change ra and rij to ra_ and rij_
          if (Pj > Pj_max) {
            Pj_max = Pj;
            b_max = b; //**
          }
...
//        for (size_t a = 0; a < si.size(); a++) {
//      Site& sa = si.at(a);
    int type_sq = pj.type * pj.type + pi.type * pi.type;
...
//      if (sa.type + sb.type ==0 ) {
    ...
    //ya = sa.y - pi.y;
    //za = sa.z - pi.z;
    xa = pi.sites[a_max].x - pi.x;
    ya = pi.sites[a_max].y - pi.y;
    za = pi.sites[a_max].z - pi.z;
...
//    zb = sb.y - pj.z;
    ...
    xb = pj.sites[b_max].x - pj.x;
    yb = pj.sites[b_max].y - pj.y;
    zb = pj.sites[b_max].z - pj.z;
    ...
    rijLsq = xij * xij + yij * yij + zij * zij;
    rijL = sqrt(rijLsq); // rij is the distance between two particles 

    rijLInv1 = 1 /rijL;
    rijLInv2 = 1 / rijLsq;
    rijLInv6 = rijLInv2 * rijLInv2 * rijLInv2;
    rijLInv12 = rijLInv6 * rijLInv6;


    D3Vector<double> ra( xa, ya, za ), rb( xb, yb, zb), rij( xij, yij, zij), Fj(0,0,0),Tri(0,0,0),Trj(0,0,0);
     ...
     g_theta1 = 1 / ((exp(alpha * (P_0-Pi))) + 1) ; //angle (theta1) dependence on particle i 
     g_theta2 = 1 / ((exp(alpha * (P_0-Pj))) + 1) ; //angle (theta2) dependence on particle j

     Vata = - (64 * epsi * rijLInv6 * g_theta1 * g_theta2); //attractive potential
     Vrep = epsiRep * exp(-B1 *(rijL-2));



Fj =  (((((rij.scalarp(Pi).subs(ra)).scalarp(1/(1 + (exp(alpha * (Pi-P_0)))))).addi(((rij.scalarp(Pj).addi(rb)).scalarp(1/(1 + (exp(alpha * (Pj-P_0)))))))).scalarp(alpha)).addi(rij.scalarp(6))).scalarp(Vata/rijL).addi(rij.scalarp(B1).scalarp(Vrep));  //march 11

    Tri  = ((rij.crossp(ra)).scalarp(alpha * Vata)).scalarp(1 / (1 + (exp(alpha * (Pi-P_0)))));
  
    Trj  = ((rb.crossp(rij)).scalarp(alpha * Vata)).scalarp(1 / (1 + (exp(alpha * (Pj-P_0)))));
...
void ParticleCollection::computeNonPatchForcesAndTorques(const PairList pairs_, double side_,size_t max_site_type_
                                                     ) {
  double xab,xa,xb,xij;
  double yab,ya,yb,yij;
  double zab,za,zb,zij;
  ...
  double faby,fijy;
  double fabz,fijz;
  double theta_1, theta_2, Vnp, Vrep, M, g_theta1, g_theta2;
  double alpha, P_0, Pi,Pj,epsiNP,epsiRep,B1,B;
...
//      if (sa.type + sb.type ==0 ) {
    ...
    D3Vector<double> ra( xa, ya, za ), rb( xb, yb, zb), rij( xij, yij, zij), Fj(0,0,0),Tri(0,0,0),Trj(0,0,0);
     ...
     Vnp = - (64 * epsiNP * rijLInv6 * (1 - g_theta1) * (1 - g_theta2)); //attractive potential
     Vrep = epsiRep * exp(-B1 *(rijL-2));


      Fj =  ((((((ra.subs(rij.scalarp(Pi))).scalarp(1/(1 + (exp(alpha * (P_0-Pi)))))).subs((rij.scalarp(Pj).addi(rb)).scalarp(1/(1 + (exp(alpha * (P_0-Pj))))))).scalarp(alpha)).addi(rij.scalarp(6))).scalarp(Vnp/rijL)).addi(rij.scalarp(B1).scalarp(Vrep)); //march 14



    Tri  = ((ra.crossp(rij)).scalarp(alpha * Vnp)).scalarp(1 / (1 + (exp(alpha * (P_0-Pi)))));

    Trj  = ((rij.crossp(rb)).scalarp(alpha * Vnp)).scalarp(1 / (1 + (exp(alpha * (P_0-Pj)))));
...
void ParticleCollection::computeExcludedVolumeForces(const PairList pairs_, double side_,
  ...
  for (PairList::const_iterator it = pairs_.begin(); it < pairs_.end(); it++) {
    ...
    fmagn_over_rij = epsi * B * exp(-B * surface_to_surface) / rij;
...
//             pow(p.fz * p.b * dt_ + rndz, 2)) / pow(dt_, 2);
   ...
   //} changed for not roration of type -1
   } //end first IF 
    }  //end else 
    ...
    }    // END 2nd IF statement for particle type 
...
void ParticleCollection::computeAngles(long long t_ ,double dt_,const PairList pairs_, double side_,size_t max_site_type_
                                                     ) {
...
//      if (sa.type + sb.type ==0 ) {
  ...
  std::ofstream ratio;
  ratio.open("angleStats.dat", std::ios::app);
  ratio << t_ * dt_ << "          " << Pi << "       " <<Pj <<"   " << theta_1 << "    " <<theta_2 << "  " << rijL<<  std::endl;
  ratio.close();
...
void ParticleCollection::writeContactsAB(long long t_, double dt_, double side_) {
double xi;
double yi;
double zi;
double xij;
...
int type_sq = pi.type * pi.type + pj.type * pj.type;
// if (type_sq == 0 || type_sq == 4 || type_sq == 8) {
 if (type_sq == 4) {
      ...
      mic(xij, yij, zij, side_);
      rijsq = xij * xij + yij * yij + zij * zij;
      if(sqrt(rijsq) - radiusi - radiusj < 0.50){ contacts++;
      if(pi.type != 1 && pi.type != -1) pi.contacts++; // new line niladri 
      if(pj.type != 1 && pj.type != -1) pj.contacts++; // new line niladri
    }
    } //end if on particle types
  }// end for loop over j
}// end for loop over i
...
for (ParticleCollection::iterator it = this->begin(); it < this->end(); it++) {
  Particle& p = *it;
  if (p.type != 1 && p.type != -1) {
   if (p.contacts < 20) hist[p.contacts]++;
   else std::cout << "ERROR: free particle with more than 14 surface contacts"<< std::endl;
    }
    p.contacts = 0; // reset to zero for next time
} // end loop over individual particles 
std::ofstream agg;
agg.open("contactsAB.dat", std::ios::app);
for (int i=0; i<20; i++)
agg << t_*dt_ <<" " << i << " " <<hist[i]<<std::endl;
agg.close();
}

void ParticleCollection::writeContactsAAorBB(long long t_, double dt_, double side_) {
double xi;
double yi;
double zi;
double xij;
...
// if (type_sq == 4) {
      ...
      mic(xij, yij, zij, side_);
      rijsq = xij * xij + yij * yij + zij * zij;
      if(sqrt(rijsq) - radiusi - radiusj < 0.5){ contacts++;
      if(pi.type != 1 && pi.type != -1) pi.contacts++; // new line niladri 
      if(pj.type != 1 && pj.type != -1) pj.contacts++; // new line niladri
    }
    } //end if on particle types
...
std::ofstream agg;
agg.open("contactsAAorBB.dat", std::ios::app);
for (int i=0; i<20; i++)
agg << t_*dt_ <<" " << i << " " <<hist[i]<<std::endl;
agg.close();
}
...
void ParticleCollection::writeContactsALL(long long t_, double dt_, double side_) {
double xi;
double yi;
double zi;
double xij;
...
int type_sq = pi.type * pi.type + pj.type * pj.type;
 if (type_sq == 0 || type_sq == 8 || type_sq == 4) {
// if (type_sq == 4) {
...
std::ofstream agg;
agg.open("contactsALL.dat", std::ios::app);
for (int i=0; i<20; i++)
agg << t_*dt_ <<" " << i << " " <<hist[i]<<std::endl;
agg.close();
}
...
void ParticleCollection::writeContactsABandAAorBB(long long t_, double dt_, double side_) {
double xi;
double yi;
double zi;
double xij;
...
double radiusi;
double radiusj;
long contactsAB = 0; // contacts between differnt types of particles
long contactsAAorBB = 0; //contacts between same type of particles 
// loop over pairs of particles
for (unsigned int i = 0; i < this->size() - 1; i++) {
   Particle& pi = this->at(i);
  xi = pi.x;
  ...
  for (unsigned int j = i + 1; j < this->size(); j++) {
   Particle& pj = this->at(j);
  int type_sq = pi.type * pi.type + pj.type * pj.type;
//for different type of particles contacts
  if (type_sq == 4) {
      xij = xi - pj.x;
      yij = yi - pj.y;
      zij = zi - pj.z;
      ...
      mic(xij, yij, zij, side_);
      rijsq = xij * xij + yij * yij + zij * zij;
      if(sqrt(rijsq) - radiusi - radiusj < 0.4){ contactsAB++;
      if(pi.type != 1 && pi.type != -1) pi.contactsAB++; // new line niladri 
      if(pj.type != 1 && pj.type != -1) pj.contactsAB++; // new line niladri
      }
    } //end if on AB type particle types
//for same type of particles contacts
  if (type_sq == 0 || type_sq == 8) {
      ...
      mic(xij, yij, zij, side_);
      rijsq = xij * xij + yij * yij + zij * zij;
      if(sqrt(rijsq) - radiusi - radiusj < 0.4){ contactsAAorBB++;
      if(pi.type != 1 && pi.type != -1) pi.contactsAAorBB++; // new line niladri 
      if(pj.type != 1 && pj.type != -1) pj.contactsAAorBB++; // new line niladri
    }
   } //end if on AA or BB particle types
  }// end for loop over j
}// end for loop over i
const int const_max =20;
int hist[const_max][const_max];

 // new line to define 2-d array
for (int m=0; m<const_max; m++)  
for (int n =0;n<const_max; n++)
  hist[m][n]=0;
for (ParticleCollection::iterator it = this->begin(); it < this->end(); it++) {
  Particle& p = *it;
// do not count the surface particles (type = 1 and type= -1)
  if (p.type != 1 && p.type != -1) {
   if (p.contactsAB < const_max && p.contactsAAorBB < const_max) hist[p.contactsAB][p.contactsAAorBB]++;
   else std::cout << "ERROR: free particle with more than" << const_max << "surface contacts"<< std::endl;
    }
    p.contactsAB = 0; // reset to zero for next time
    p.contactsAAorBB = 0; // reset to zero for next time
} // end loop over individual particles 
std::ofstream agg;
agg.open("contactsALL.dat", std::ios::app);
for (int m=0; m<const_max; m++)
for (int n =0;n<const_max; n++)
agg << t_*dt_ <<" " << "hist["<< m << "][ " << n << "] " <<hist[m][n]<<std::endl;
agg.close();
}
